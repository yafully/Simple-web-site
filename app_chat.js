...
server.use(function(socket, next) {
	...
	if (data.headers.cookie) {
		...
		if(typeof data.cookie['exSid'] != 'undefined'){
			
			try{
				data.sessionID = cookieParser.signedCookie(data.cookie['exSid'], 'secret');
...
server.on('connection',function(socket){   // server listening
	...
	let session = socket.handshake.session;//获取socket里面的session


	if(typeof session.chatauth =='undefined'){//session丢失
		socket.broadcast.to(roomId).emit("logout");
		console.log('socket offline!');

	}else{
		socket.broadcast.to(roomId).emit("login");
