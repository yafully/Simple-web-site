...
function(XHEDITOR, $, undefined) {
	...
	var arrDbClick = {
		...
		Xheditor = function(textarea, options) {
			...
			function forcePtag(a) {
				...
				//原来是"formatblock", "<p>"现在是"formatblock", "<div>"yafully
				return b.is("li") ? !0 : settings.forcePtag ? void(0 === b.length && _this._exec("formatblock", "<div>")) : (_this.pasteHTML("<br />"), isIE && b.length > 0 && 2 === _this.getRng().parentElement().childNodes.length && _this.pasteHTML("<br />"), !1)
			}
			...
			this.init = function() {
				...
				isOpera ? n.bind("keydown", function(a) {
					...
					if (d && (c = d.files) && c.length > 0) {
						...
						//console.log(d.files);
						//onFileDrop
						//$.fireEvent(settings.uploadCompelete, [data.msg,data.err]);
						...
						return 0 === i.length ? !1 : (g = i.join(","), f = b(c), 1 === f ? alert(getLang("upload.extLimit", g.replace(/\w+:,/g, ""))) : 2 === f ? alert(getLang("upload.typeLimit")) : f && _this.startUpload(c, settings["up" + f + "Url"], "*", function(a) {
							...
							for (var e = 0, g = a.length; g > e; e++){
								...
								"!" === url.substr(0, 1) && (url = url.substr(1)), c.push(url);
							}
							
							if($.isFunction(settings.onDropFile)&&f!="Img"){
								$.fireEvent(settings.onDropFile, [_this,f,c,$("#xheSave"),a]);
							}else{
				...
				var c = arrEmots[a],
					...
					l = c.list;
				if (k)
					for (var m = 1; k >= m; m++) e++, f.push("<a href=\"javascript:void('" + m + '\')" style="background-image:url(' + d + m + '.gif);" emot="' + a + "," + m + '" xhev="" title="' + m + '" role="option">&nbsp;</a>'), e % j === 0 && f.push("<br />");
				else {
					var n = is(l, "array");
					$.each(l, function(b, c) {
			...
			}, this.startUpload = function(fromFiles, toUrl, limitExt, onUploadComplete,objType) {
				function onUploadCallback(sText, bFinish) {
					var data = Object,
						bOK = !1;
					try {
						data = eval("(" + sText + ")");
						$.fireEvent(settings.uploadCompelete, [data.msg,data.err,objType]);
					} catch (ex) {}
				...
				if (!isOpera && bHtml5Upload && (!fromFiles.nodeType || (fileList = fromFiles.files) && fileList[0].name)) {
					...
					for (i = 0; len > i; i++){
						...
						if (!checkFileExt(fileList[i].name, limitExt)) return;//检测
					}
					...
					if (!checkFileExt(fromFiles.value, limitExt)) return;//检测
	...
	XHEDITOR.settings = {
		...
		showBlocktag: !1,
		forcePtag: true,
		upLinkExt: "zip,rar,txt,doc,docx,ppt,pptx,xls,pdf",
		...
		return this.each(function() {
			...
			if (this.xheditor) arrSuccess.push(this.xheditor);
			else {
