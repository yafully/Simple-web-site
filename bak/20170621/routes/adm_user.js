...
//const adminFnModel = require('../modules/adminFnSetup');//菜单数据表
//const guestModel = require('../modules/guestSetup');
const pageHelper = require('../helper/pager');
//const getAdminMenu = require('../helper/adminMenu');
...
app.get('/lwht', function(req, res, next) {
    ...
    //获取sidebar数据
    // getAdminMenu(adminFnModel,function(error, data){
        
        
    // });
    response.render('backend/admin/index',{
        title:'乐薇网站管理系统',
...
app.post('/login', function (req, res, next) {
    ...
    adminModel.findOne({'name':uname},function(err,doc){   //通过此model以用户名的条件 查询数据库中的匹配信息  
        ...
        if(err){                                         //错误就返回给原post处（login.html) 状态码为500的错误  
            ...
            doc.comparePassword(upwd, (err, isMatch) => {
                if (isMatch && !err) {
                    req.session.regenerate(function(){
                        req.session.user = doc.name;
                        req.session.email = doc.email;
